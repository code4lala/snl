		...
		int parse(const char *code) {
			...
			while (*code != '\0') {
				...
				if (r <= 0) {
					...
					// 出错
					switch (r) {
					case ERROR_COMMENT:			errMsg += "少一个右大括号导致注释没有闭合";	return E_COMMENT_NOT_TERMINATED;
					case ERROR_TWO_CHAR:		errMsg += "冒号该有等号却没有";			return E_ONLY_COLON_BUT_NO_EQUAL;
					case ERROR_UNEXPECTED_CHAR:	errMsg += "遇到了snl文法规定以外的符号";	return E_UNEXPECTED_CHAR;
					}
					return E_UNKNOWN;
				}
				else {
